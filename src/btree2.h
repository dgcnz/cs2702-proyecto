  ...
  public:
    class iterator : std::iterator<std::forward_iterator_tag, T, T, T *, T &>
    {
      ...
      public:
        ...
        long get_page_id() { return root.value().children[pos]; }
    };
    ...
    std::pair<iterator, iterator> find(const T &first, const T &ilast)
    {
        /*  Returns iterator range corresponding to [first, ilast].
         *
         * Range is represented as pair of iterators [itl, itr], such that:
         *  * itl is the first element greater or equal than first
         *  * itr is the first element greater than ilast
         */
    }
